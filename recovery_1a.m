...
daily_recovered_7d(1)=mean(daily_recovery(1:4));
daily_recovered_7d(2)=mean(daily_recovery(1:5));
daily_recovered_7d(3)=mean(daily_recovery(1:6));
for j=4:DD-3
   daily_recovered_7d(j)=mean(daily_recovery(j-3:j+3));
end
daily_recovered_7d(DD-2)=mean(daily_recovery(DD-5:DD));
daily_recovered_7d(DD-1)=mean(daily_recovery(DD-4:DD));
daily_recovered_7d(DD)=mean(daily_recovery(DD-3:DD));

 cumulative_recovery_7d(1)=daily_recovered_7d(1);
 for i=2:length(daily)
    cumulative_recovery_7d(i)= cumulative_recovery_7d(i-1)+daily_recovered_7d(i);
 end

 %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
daily_recovered_7d(71)=daily_recovered_7d(70);
daily_recovered_7d(78)=daily_recovered_7d(75);
for i=1:length(daily_recovered_7d)
         if daily_recovered_7d(i) <0
             daily_recovered_7d(i)=0;
         end
     end


daily_recovered_7d1=daily_recovered_7d(1:end);
daily_7d1=daily_7d(1:end);

%     K=daily_recovered_7d1(1:150)/59100000;
K=daily_recovered_7d1;
h=1;
    N=length(K);
    NN=1:h:N;
    hn=N^(-1/5);
...
GG1=0.97*gampdf(NNN,aa1,bb1);
    ...
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    MM1=mean(active_7d*(0.97/(Mode)));
SS1=std(active_7d*(0.97/(Mode)));
MS1=MM1-3*SS1;
xconf1 = [1:1:length(active_7d) length(active_7d):-1:1] ;         
yconf1 = [active_7d*(0.97/(Mode))+MS1 active_7d(end:-1:1)*(0.97/(Mode))-MS1];
%fill(xconf1,yconf1,'m');
fill(xconf1,yconf1,'m','FaceAlpha',0.4,'LineStyle','none');hold on

MM2=mean(active_7d*(0.97/(Median)));
SS2=std(active_7d*(0.97/(Median)));
MS2=MM2-3*SS2;
xconf2 = [1:1:length(active_7d) length(active_7d):-1:1] ;         
yconf2 = [active_7d*(0.97/(Median))+MS2 active_7d(end:-1:1)*(0.97/(Median))-MS2];
%fill(xconf1,yconf1,'m');
fill(xconf2,yconf2,'r','FaceAlpha',0.6,'LineStyle','none'); hold on

MM3=mean(active_7d*(0.97/(Mean)));
SS3=std(active_7d*(0.97/(Mean)));
MS3=MM3-3*SS3;
xconf3 = [1:1:length(active_7d) length(active_7d):-1:1] ;         
yconf3 = [active_7d*(0.97/(Mean))+MS3 active_7d(end:-1:1)*(0.97/(Mean))-MS3];
%fill(xconf1,yconf1,'m');
...
plot(K,'k.','MarkerSize',20); hold on 
 ...
 plot(active_7d*(0.97/(Mode)),'m','LineWidth',2); hold on 
   plot(active_7d*(0.97/(Mean)),'g','LineWidth',2); hold on
    plot(active_7d*(0.97/(Median)),'r','LineWidth',2); hold on
...
ylabel('Daily recovery', 'interpreter','latex');

    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% MM1=mean(Cactive_7d*(0.97/(Mode)));
% SS1=std(Cactive_7d*(0.97/(Mode)));
% MS1=MM1-3*SS1;
% xconf1 = [1:1:length(Cactive_7d) length(Cactive_7d):-1:1] ;         
% yconf1 = [Cactive_7d*(0.97/(Mode))+MS1 Cactive_7d(end:-1:1)*(0.97/(Mode))-MS1];
% %fill(xconf1,yconf1,'m');
% fill(xconf1,yconf1,'m','FaceAlpha',0.4,'LineStyle','none');hold on
% 
% MM2=mean(Cactive_7d*(0.97/(Median)));
% SS2=std(Cactive_7d*(0.97/(Median)));
% MS2=MM2-3*SS2;
% xconf2 = [1:1:length(Cactive_7d) length(Cactive_7d):-1:1] ;         
% yconf2 = [Cactive_7d*(0.97/(Median))+MS2 Cactive_7d(end:-1:1)*(0.97/(Median))-MS2];
% %fill(xconf1,yconf1,'m');
% fill(xconf2,yconf2,'r','FaceAlpha',0.6,'LineStyle','none'); hold on
% 
% MM3=mean(Cactive_7d*(0.97/(Mean)));
% SS3=std(Cactive_7d*(0.97/(Mean)));
% MS3=MM3-3*SS3;
% xconf3 = [1:1:length(Cactive_7d) length(Cactive_7d):-1:1] ;         
% yconf3 = [Cactive_7d*(0.97/(Mean))+MS3 Cactive_7d(end:-1:1)*(0.97/(Mean))-MS3];
% %fill(xconf1,yconf1,'m');
% fill(xconf3,yconf3,'g','FaceAlpha',0.4,'LineStyle','none'); hold on
% 
% 
...
%  plot(Cactive_7d*(0.97/(Mode)),'m','LineWidth',2); hold on 
%    plot(Cactive_7d*(0.97/(Mean)),'g','LineWidth',2); hold on
%     plot(Cactive_7d*(0.97/(Median)),'r','LineWidth',2); hold on
% axis([0 243 0 8*10^6])
% 
%  xticks([0 121 243]);
%   xticklabels({'1/10/2020','30/1/2021','31/5/2021'});
% xlabel('Days', 'interpreter','latex');
% ylabel('Cumulative recovery', 'interpreter','latex');
